...
export default function WeekCard({ weekNumber, topics, resources, miniProjects, isLast }: WeekCardProps) {
  return (
    <div className="relative pl-10">
      {/* Waypoint node */}
      <div className="absolute left-0 top-1 w-5 h-5 rounded-full bg-[var(--accent)] flex items-center justify-center">
      ...
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-5 mb-8">
        ...
        <div className="mt-3">
          ...
          <div className="flex flex-wrap gap-2">
            {topics.map((t) => (
              <span key={t} className="text-xs bg-[var(--surface-hover)] border border-[var(--border)] rounded-full px-3 py-1 text-[var(--text)]">
                {t}
              </span>
            ))}
          </div>
